...
import logo from './TTiD-Wsmall.svg';
function Base() {
	...
	function calculateStateTaxRate() {
		...
		if (state.includes(usState)) {
			...
			if (taxIncome < deduction) {
				...
				totalStateTax =
					stateTaxRate1 * stateBracket1 + stateBracket2 * stateTaxRate2;
				...
				totalStateTax =
					stateBracket1 * stateTaxRate1 +
					stateBracket2 * stateTaxRate2 +
					stateBracket3 * stateTaxRate3;
				setStateTax(totalStateTax);
				setStateTaxRate((totalStateTax / taxIncome) * 100);
			} else if (
				fileStatus === 'Single' &&
	...
	function calculateFedTax() {
		...
		if (
			...
			if (fileStatus === 'Single' && taxIncome >= 523601) {
				...
				federalTax = fedBracket2 * fedTaxRate2 + fedBracket1 * fedTaxRate1;
				takeHome = grossIncome - federalTax - stTax - fica - medicare;
				setFedTax(federalTax);
				setFedTaxRate((federalTax / grossIncome) * 100);
				setTax(grossIncome - takeHome);
		...
		<>
			<div className='nav'>
				<div className='title-div'>
					<img src={logo} alt='TTiDesign Logo' className='logo'/>
					<p className='title'>Designed and built by TTiDesign </p>
				</div>
			</div>
				...
				<div className='left-div'>
					...
					<div className='break'>
						...
						<p className='break-down'>
							...
							<span>
								$
								{taxableIncome.toLocaleString('en', {
									maximumFractionDigits: 2,
								})}
							</span>
						</p>
						...
						<p className='break-down'>
							...
							<span>
								{stateTaxRate.toLocaleString('en', {
									maximumFractionDigits: 2,
								})}
								%
							</span>
						...
						<p className='break-down'>
							...
							<span className='high-light'>
								$
								{FinalTakeHome.toLocaleString('en', {
									maximumFractionDigits: 2,
								})}
							</span>
						</p>
					...
					<p>
						*These calculations do not take into consideration of subtractions
						from <br></br>
